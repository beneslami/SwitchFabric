...
int nr_events ;
/*---------------------------------------------------------------------------*/
EVENT *new_event(void (*funct)(), char *data)
{
  EVENT *event ;
...
void check_events()
{
  ...
  i = 0 ;
  if(!graph)printf("-checking for %d events-",nr_events) ; 
...
void process_events()
{
  ...
  FOREACH(event, events) {
    if(debug & DEBUG_EVENTS) {
      printf("EVENT %d(%d)\n",(int)event->funct,(int)event->data) ;
    }
    (*event->funct)(event->data) ;
  }
