...
void free_message(MESSAGE *message)
{
  ...
  message->M_DR = NULL ;
  message->chang_dir_nr = NULL ;
  message->length = message->state = message->start_time = NULL ;
  message->head_lane = NULL_LANE ;
...
/*---------------------------------------------------------------------------*/
void eliminate_the_message(MESSAGE *message) 
{
  LANE *to_lane, *from_lane ;

  to_lane = message->head_lane ;
  while(to_lane) {
    from_lane = to_lane->prev ;
      if(to_lane != NULL_LANE) {
	set_lane_idle(to_lane) ;
      }
    to_lane = from_lane ;
  }
  set_message_done(message) ;
}
/*---------------------------------------------------------------------------*/
/*
 |    Request lane
...
void allocate_next_lane(CHANNEL *channel)
{
  ...
  while(requests) {
    ...
    lane = request->lane ;
    chan_state = lane->channel->state ;
    if(lane->state != OCCUPIED && chan_state != DISCON && 
       chan_state != FAILED) {
      assign_lane_to_message(lane, request->message) ;
    }
    else if(chan_state != DISCON && chan_state != FAILED) {
...
void allocate_channels_for_flits()
{
  ...
  FOREACH(channel, active_channels) {
	...
	printf("chanel = %x  Channel Id = %d\n", channel, channel->dst_slot) ;
	print_channel_states(channel->src->id_seq) ;
	error(" program error at allocate_channels_for_flits") ;
      }
    }
