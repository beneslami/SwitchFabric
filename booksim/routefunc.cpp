...
void single( const Router *, const Flit *f, int, OutputSet *outputs, bool inject )
{
  outputs->Clear( );
...
void planar_adapt_mesh( const Router *r, const Flit *f, int in_channel, OutputSet *outputs, bool inject )
{
  ...
  if ( cur != dest ) {
	...
	cout << "ERROR: There seem to be faults in d_i and d_{i+1} "
	     << "at node " << r->GetID( ) << "!" << endl;
	exit(-1);
      }
...
void InitializeRoutingMap( )
{
  ...
  gRoutingFunctionMap["single_single"]   = &single;
